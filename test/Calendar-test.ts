...
describe("Calendar", function() {
  ...
  const year = new Date().getFullYear(), month = 12, day = 31;
  ...
  it("books meetings with others within the available hours", async function() {
    ...
    const res0 = await cal2.getMeetings(year, month, 31);
    chai.expect(res0).to.be.instanceof(Array);
    chai.expect(res0).to.have.length(0);

    const [hour1, min1] = [14, 15], duration = 60;

    await cal2.connect(signer1).bookMeeting(year, month, day, hour1, min1, duration);
    const res1 = await cal2.getMeetings(year, month, 31);
    ...
    await cal2.connect(signer1).bookMeeting(year, month, day, hour2, min2, duration);
    const res2 = await cal2.getMeetings(year, month, 31);
    ...
    it("reverts on cancelling non-existing meetings", async function() {
      let id = 0;
      await chai.expect(cal2.connect(signer1).cancelMeeting(year, month, day, id)
  ...
  it("reverts on invalid date and time", async function() {
    await chai.expect(
      cal2.connect(signer1).bookMeeting(year, 2, 29, month, 0, 60)
    ).to.be.revertedWith("Date and time are not valid.");

    await chai.expect(
      cal2.connect(signer1).bookMeeting(year, 2, 28, 24, 0, 10)
    ).to.be.revertedWith("Date and time are not valid.");
  });
    ...
    it("prohibits to book a meeting overlapping with an earlier meeting", async function() {
      ...
      await chai.expect(
        cal1.connect(signer3).bookMeeting(year, month, day, 14, 30, 60)
    ...
    it("prohibits to book a meeting overlapping with a later meeting", async function() {
      await cal1.connect(signer2).bookMeeting(year, month, day, 14, 0, 60);
      await chai.expect(
        cal1.connect(signer3).bookMeeting(year, month, day, 13, 30, 60)
      ).to.be.revertedWith("Overlap with existing meeting.");
    });
    ...
    it("prohibits to book a meeting falling inside of another meeting", async function() {
      await cal1.connect(signer2).bookMeeting(year, month, day, 14, 0, 60);
      await chai.expect(
        cal1.connect(signer3).bookMeeting(year, month, day, 14, 20, 20)
      ).to.be.revertedWith("Overlap with existing meeting.");
    });
    ...
    it("prohibits to book a meeting surrounding another meeting", async function() {
      await cal1.connect(signer2).bookMeeting(year, month, day, 14, 0, 60);
      await chai.expect(
        cal1.connect(signer3).bookMeeting(year, month, day, 13, 0, 180)
      ).to.be.revertedWith("Overlap with existing meeting.");
    });

    it("prohibits to book a meeting that overlaps with a meeting from the previous day", async function() {
      await cal3.connect(signer2).bookMeeting(year, month, day, 23, 0, 120);
      await chai.expect(
        cal3.connect(signer2).bookMeeting(year+1, 1, 1, 0, 30, 60)
  ...
  it("prohibits to book a meeting that overlaps with a meeting from the next day", async function() {
    ...
    await chai.expect(
      cal3.connect(signer2).bookMeeting(year, month, day, 23, 0, 120)
    ...
    it("prohibits cancelling meetings of others", async function() {
      ...
      let id = 0;
      await chai.expect(cal2.connect(signer2).cancelMeeting(year, month, day, id)
      ).to.be.revertedWith(
    ...
    it("prohibits booking meetings with yourself", async function() {
      await chai.expect(
        cal2.connect(signer2).bookMeeting(year, month, day, 14, 15, 60)
